import { ChangeDetectorRef, Component, Host, HostBinding, Input, NgModule, OnDestroy, OnInit } from '@angular/core';
...
@Component({
  ...
  template: `
		<ng-container>
            <div *ngIf="root && item.visible !== false" class="layout-menuitem-root-text">{{item.label.replace('\n', '&#8249br&#8250')}}</div>
			<a *ngIf="(!item.routerLink || item.items) && item.visible !== false" [attr.href]="item.url" (click)="itemClick($event)"
			   [ngClass]="item.class" [attr.target]="item.target" tabindex="0" pRipple>
				<i [ngClass]="item.icon" class="layout-menuitem-icon"></i>
				<span class="layout-menuitem-text">{{item.label.replace('\n', '&#8249br&#8250') }}</span>
